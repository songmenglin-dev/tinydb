...
def validate_columns(
    ...
    * REQ-JOIN-8: bare column names that exist in multiple joined tables
    ...
    _walk_expr(expr, alias_map, aliased_tables, tables_with_col)
...
def _walk_expr(
    ...
    if isinstance(expr, ColumnRef):
        if expr.table is not None:
            ...
            if expr.table not in alias_map.aliases:
                ...
                if expr.table in tables_with_col:
        ...
        owners: list = []
        for canonical_name in tables_with_col.keys():
            owners.append(canonical_name)
        if len(owners) > 1 and len(_tables_in_from(tables_with_col)) > 1:
            if _is_join_context(tables_with_col):
    ...
    if isinstance(expr, BinaryOp):
        _walk_expr(expr.left, alias_map, aliased_tables, tables_with_col)
        _walk_expr(expr.right, alias_map, aliased_tables, tables_with_col)
...
def emit_logical(stmt: Select) -> LogicalPlan:
