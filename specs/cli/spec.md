...
## v0.3 增量：CLI 双模式（embedded + remote，REQ-CLI-CS-1 ~ REQ-CLI-CS-8）

> **合并说明**：本节是 `changes/tinydb-v0.3/specs/cli-cs/spec.md`（v0.3 ADDED 要求）的合并版本。REQ 编号 `REQ-CLI-CS-N` 与 delta 一致。
>
> **依赖**：本节叠加在 v0.2 CLI 增强之上，不破坏既有 `CLI-BASE-*` 与 `CLI-V2-*` 行为。

### REQ-CLI-CS-1: 启动模式选择
The system MUST 在 `tinydb` 命令行入口接受 `--file PATH` 或 `--uri URI`（互斥）；`--file` 走 embedded（直接 `tinydb.api.Database`），`--uri` 走 remote（`tinydb.client.Client`）；两者都未给时默认 embedded 并要求后续 `open <path>` 元命令。
...
- AND `.tables` 经由 `Client.execute("...")` 派发
