...
> **说明**：在 `db.execute()` API 层，`tinydb` 支持完整 BEGIN / COMMIT / ROLLBACK 事务（见 `scripts/functional_tests.py` 的事务类别 100% 通过）。REPL 单条 SQL 路径走 autocommit 模式——事务由 `with db.transaction():` 块 API 触发，不是 REPL 内联 SQL。
